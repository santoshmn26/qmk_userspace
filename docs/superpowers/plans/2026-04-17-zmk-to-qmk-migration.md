# ZMK to QMK Keymap Migration Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Translate the ZMK Microdox v2 keymap into a new QMK keymap (`santu`) for the Charybdis Nano 3x5 with all 5 layers, 33 combos, tap-dance, and mod-tap preserved.

**Architecture:** Create a new keymap folder `keyboards/bastardkb/charybdis/3x5/keymaps/santu/` with three files: `config.h` (timing), `rules.mk` (feature flags), and `keymap.c` (all logic). Two ZMK combos that share the same key positions across layers are handled via custom keycodes + `process_record_user`. Thumb row shrinks from 6 to 5 keys (rightmost `LALT` dropped).

**Tech Stack:** QMK firmware C, Charybdis Nano 3x5 keyboard target (`bastardkb/charybdis/3x5`)

---

## Key Position Reference

Both keyboards use a 3×5 grid, so positions 0–29 map identically:
```
 Q(0)  W(1)  E(2)  R(3)  T(4)    Y(5)  U(6)  I(7)  O(8)  P(9)
A(10) S(11) D(12) F(13) G(14)   H(15) J(16) K(17) L(18) ENT(19)
Z(20) X(21) C(22) V(23) B(24)   N(25) M(26) ,(27) .(28) /(29)
            LGUI(30) NAV(31) SPC(32)   SHIFT(33) SYM(34)
```

## ZMK → QMK Keycode Translation

| ZMK | QMK |
|---|---|
| `mt LCTRL Z` | `LCTL_T(KC_Z)` |
| `lt 4 FSLH` | `LT(LAYER_SHORTCUT, KC_SLSH)` |
| `lt 3 SPACE` | `LT(LAYER_NUM, KC_SPC)` |
| `mo 1` / `mo 2` | `MO(LAYER_NAV)` / `MO(LAYER_SYM)` |
| `td1` (shift/caps) | `TD(TD_SHIFT_CAPS)` |
| `kp KP_PLUS` | `KC_KP_PLUS` |
| `kp KP_MULTIPLY` | `KC_KP_ASTERISK` |
| `kp SQT` | `KC_QUOT` |
| `kp DQT` | `KC_DQUO` |
| `kp COLON` | `KC_COLN` |
| `kp QMARK` / `kp LS(FSLH)` | `KC_QUES` |
| `kp UNDER` | `KC_UNDS` |
| `kp BSLH` | `KC_BSLS` |
| `kp TILDE` | `KC_TILD` |
| `kp GRAVE` | `KC_GRV` |
| `kp LPAR/RPAR` | `KC_LPRN` / `KC_RPRN` |
| `kp LBRC/RBRC` | `KC_LCBR` / `KC_RCBR` |
| `kp LBKT/RBKT` | `KC_LBRC` / `KC_RBRC` |
| `kp DOLLAR/PRCNT/CARET/AMPS/HASH` | `KC_DLR` / `KC_PERC` / `KC_CIRC` / `KC_AMPR` / `KC_HASH` |
| `kp SEMI` | `KC_SCLN` |
| `kp AT` | `KC_AT` |
| `kp KP_N0..N5` | `KC_P0..KC_P5` |
| `kp LC(LA(TAB))` | `LCTL(LALT(KC_TAB))` |
| `kp LG(LC(RIGHT))` | `LGUI(LCTL(KC_RGHT))` |
| `kp LG(LC(LEFT))` | `LGUI(LCTL(KC_LEFT))` |
| `kp LS(LC(LEFT/RIGHT))` | `LSFT(LCTL(KC_LEFT/RGHT))` |
| `MEH(kc)` | `MEH(KC_x)` (native QMK) |
| `mkp LCLK/RCLK` | `KC_BTN1` / `KC_BTN2` |
| `mmv MOVE_*` | `XXXXXXX` |
| `msc SCRL_UP/DOWN` | `MS_WHLU` / `MS_WHLD` |
| `msc SCRL_LEFT/RIGHT` | `XXXXXXX` |
| `bt BT_SEL *` / `bt BT_CLR` | `XXXXXXX` |
| `kp LOCKING_NUM` | `KC_NUM_LOCK` |

## Conflicting Combos (same keys, different layers)

Two ZMK combo pairs share the same key positions but are layer-restricted. Since QMK combos are global by default, these are handled with custom keycodes + `process_record_user` layer checks:

| Custom Keycode | Keys | Layer 0 action | Other layer action |
|---|---|---|---|
| `AT_OR_HIDE` | W + E | `KC_AT` | `LCTL(KC_B)` on LAYER_NAV |
| `AND_OR_DOT` | `,` + `.` | `KC_AMPR` | `KC_DOT` on LAYER_NUM |
...
**Files:**
- Create: `keyboards/bastardkb/charybdis/3x5/keymaps/santu/rules.mk`
- Create: `keyboards/bastardkb/charybdis/3x5/keymaps/santu/config.h`
- Modify: `qmk.json`

- [ ] **Step 1: Create rules.mk**

```
keyboards/bastardkb/charybdis/3x5/keymaps/santu/rules.mk
```
...
```
keyboards/bastardkb/charybdis/3x5/keymaps/santu/config.h
```
...
- [ ] **Step 3: Add santu to qmk.json build targets**

Edit `qmk.json` to add the santu keymap:
```json
{
    "userspace_version": "1.0",
    "build_targets": [
        ["bastardkb/charybdis/3x5", "vendor"],
        ["bastardkb/charybdis/3x5", "santu"]
    ]
}
```
...
```bash
cd /Users/santoshnagaraja/santu/personal_projects/migrate_zmk_to_qmk/qmk_userspace
git add keyboards/bastardkb/charybdis/3x5/keymaps/santu/rules.mk
git add keyboards/bastardkb/charybdis/3x5/keymaps/santu/config.h
git add qmk.json
git commit -m "feat: add santu keymap scaffolding for Charybdis Nano 3x5"
```

---

## Task 2: Create keymap.c with all layers

**Files:**
- Create: `keyboards/bastardkb/charybdis/3x5/keymaps/santu/keymap.c`

- [ ] **Step 1: Create keymap.c with layers, tap-dance, and custom keycodes**

```
keyboards/bastardkb/charybdis/3x5/keymaps/santu/keymap.c
```
...
- [ ] **Step 2: Verify it compiles (layers only — combos not added yet)**

```bash
cd /Users/santoshnagaraja/santu/personal_projects/migrate_zmk_to_qmk/qmk_userspace
qmk compile -kb bastardkb/charybdis/3x5 -km santu
```

Expected: Build succeeds with no errors. Fix any keycode name errors before continuing (common: `KC_DQUO` → check qmk docs, `KC_COLN` → may need `KC_COLON`).

- [ ] **Step 3: Commit**

```bash
git add keyboards/bastardkb/charybdis/3x5/keymaps/santu/keymap.c
git commit -m "feat: add santu keymap layers, tap-dance, and custom keycodes"
```
...
**Files:**
- Modify: `keyboards/bastardkb/charybdis/3x5/keymaps/santu/keymap.c`

- [ ] **Step 1: Add combo declarations after the custom keycodes enum**

Insert between `enum custom_keycodes { ... };` and `const uint16_t PROGMEM keymaps...`:
...
Expected: Build succeeds. Common issue: `KC_EXCLAIM` may need to be `KC_EXLM` depending on QMK version — check the error and fix accordingly.

- [ ] **Step 3: Commit**

```bash
git add keyboards/bastardkb/charybdis/3x5/keymaps/santu/keymap.c
git commit -m "feat: add all 33 combos to santu keymap"
```

---

## Final keymap.c structure (complete file for reference)

The final `keymap.c` reads top to bottom as:
1. `#include QMK_KEYBOARD_H`
2. `enum layers`
3. `enum tap_dance_codes`
4. `enum custom_keycodes`
5. Combo key arrays (`PROGMEM`)
6. `enum combo_events`
7. `combo_t key_combos[]`
8. `const uint16_t PROGMEM keymaps[]` (5 layers)
9. `process_record_user` (custom keycode layer checks)
